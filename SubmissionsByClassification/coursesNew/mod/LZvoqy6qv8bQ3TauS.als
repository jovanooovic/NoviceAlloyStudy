...
all c: Course, p: Project, s1: Student, s2: Student| p in s1.projects && p in s2.projects => if (c.grades[s1] > c.grades[s2]) then (c.grades[s1] - c.grades[s2]) <= 1 else (c.grades[s2] - c.grades[s1]) <= 1
