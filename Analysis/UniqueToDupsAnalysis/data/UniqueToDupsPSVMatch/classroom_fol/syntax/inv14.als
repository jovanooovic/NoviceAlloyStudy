...
-- div,1
all p : Student, some t : Teacher | (some c: Class, some g: Group | c->g->p) and t->p in Tutors
-- div,1
...
all p1 : Student | some t : Teacher | (some c: Class, some g: Group | c->g->p1) and t->p1 in Tutors
-- div,1
all sStudent,t:Teacher | (some g:Group | c->s>g in Groups) and t->c in Teaches implies t->p in Tutors
-- div,1
...
-- div,1
all s:Student | some c:Class,g:Group, t:Teacher | c->s->g in Groups and t->c in Teachers => t->s in Tutors
-- div,1
...
-- div,1
all s:Student,c:Class,t:Teacher | (some g:Group | c->s->g in Groups) and t->c in Teaches implies t->p in Tutors
-- div,1
...
-- div,1
all p : Student, c : Class, g : Group | (c->s->g in Groups => all t : Teacher | (t->c in Teaches => t->s in Tutors))
-- div,1
all s : Student, c : Class | (some g : Group |  c -> s -> g in Groups) and t -> c in Teaches implies t -> s in Tutors
-- div,1
all s:Student,c:Class | all t:Teachers | (some g:Group | c->s->g in Groups) and t->c in Teaches implies t->s in Tutors
-- div,1
all s : Student, c : Class | (some g : Group  |  c -> s -> g in Groups) and t -> c in Teaches implies t -> s in Tutors
-- div,1
